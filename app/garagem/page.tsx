    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900 p-6 rounded-3xl border border-slate-800">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Garagem Virtual</h1>
      ...
      {/* LISTA DE VEÍCULOS */}
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">Carregando sua garagem...</div>
      ) : vehicles.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {vehicles.map((v) => (
            <div
              key={v.id}
              className="bg-slate-900 p-6 rounded-2xl border border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between space-y-4 group"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-xs font-bold uppercase px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20">
                    {v.type === 'MOTO' ? <Bike className="w-3.5 h-3.5" /> : <Car className="w-3.5 h-3.5" />}
                    {v.type}
                  </span>
                  {v.plate && (
                    <span className="text-[11px] font-mono bg-slate-950 border border-slate-800 text-slate-300 px-2 py-0.5 rounded">
                      {v.plate}
                    </span>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
                <Link
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-700 text-amber-400 text-xs font-bold py-2 px-3 rounded-xl flex items-center justify-center gap-1.5 border border-slate-700 transition-colors"
                >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center space-y-4">
          <div className="w-16 h-16 rounded-2xl bg-amber-500/10 text-amber-400 flex items-center justify-center mx-auto text-2xl">
            🚗
          </div>
          <h3 className="text-lg font-bold text-white">Sua garagem está vazia</h3>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Cadastre o seu carro ou moto para agilizar a criação de orçamentos com os lojistas da Grande São Luís.
          </p>
          <button
            onClick={() => setShowModal(true)}
            className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-5 py-2.5 rounded-xl text-sm inline-flex items-center gap-2"
          >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <Car className="w-5 h-5 text-amber-400" /> Adicionar à Garagem
              </h2>
            ...
            <form onSubmit={handleAddVehicle} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all ${
                    type === 'CARRO' ? 'bg-amber-500 text-slate-950 border-amber-500' : 'bg-slate-950 text-slate-400 border-slate-800'
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all ${
                    type === 'MOTO' ? 'bg-amber-500 text-slate-950 border-amber-500' : 'bg-slate-950 text-slate-400 border-slate-800'
                  }`}
                >
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Motor</label>
                  <input
                    type="text"
                    value={engine}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2.5 px-3 text-sm text-white focus:outline-none focus:border-amber-500 uppercase"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-5 py-2.5 rounded-xl text-xs flex items-center gap-1.5"
                >
