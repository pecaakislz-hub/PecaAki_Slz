    ...
    <div className="space-y-6 py-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900 p-6 rounded-3xl border border-slate-800">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Radar de Cotações</h1>
        ...
        <Link
          href="/lojista/perfil"
          className="bg-slate-800 hover:bg-slate-700 text-amber-400 border border-amber-500/30 font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 shrink-0"
        >
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">Atualizando radar de solicitações...</div>
      ) : quotes.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {quotes.map((q) => {
            const targetCities = q.targetCities ? JSON.parse(q.targetCities) : []
            return (
              <div
                key={q.id}
                className="bg-slate-900 p-6 rounded-3xl border border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between space-y-4 group"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[10px] font-bold uppercase px-2.5 py-0.5 rounded">
                      {q.category}
                    </span>
                    <span className="text-xs text-slate-400 flex items-center gap-1">
                      <MapPin className="w-3.5 h-3.5 text-amber-400" /> Bairro {q.user?.neighborhood || 'São Luís'}
                    </span>
                  </div>
                  ...
                  {q.vehicle ? (
                    <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-xs font-medium text-slate-200">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400">
                    ...
                    <span>Propostas: <strong className="text-amber-400">{q.proposals?.length || 0}</strong></span>
                  </div>
                </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">Nenhum pedido atende ao filtro selecionado</h3>
