      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl dark:shadow-2xl space-y-6 transition-colors duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">WhatsApp / Telefone</label>
              <input
                type="tel"
                required
                value={phone}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Município da Grande SLZ</label>
              <select
                value={city}
          ...
          {role === 'LOJISTA' && (
            <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Nome Fantasia da Loja</label>
                  <input
                    type="text"
                    required
                    value={fantasyName}
                    ...
                    className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl py-2.5 px-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 transition-colors"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">CNPJ ou CPF</label>
                  <input
                    type="text"
                    required
                    value={cnpjCpf}
          ...
          <div className="text-[11px] text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
            ...
            <span>Ao se cadastrar, você concorda com nossos <Link href="/termos" className="text-amber-600 dark:text-amber-400 underline">Termos</Link> e <Link href="/privacidade" className="text-amber-600 dark:text-amber-400 underline">LGPD</Link>.</span>
          </div>
...
export default function RegisterPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 p-8 flex items-center justify-center">Carregando formulário...</div>}>
      <RegisterContent />
    </Suspense>
  )
}
