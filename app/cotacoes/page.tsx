  ...
  return (
    <div className="space-y-6 py-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900 p-6 rounded-3xl border border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Minhas Cotações</h1>
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">Buscando suas cotações...</div>
      ) : quotes.length > 0 ? (
        <div className="space-y-4">
          {quotes.map((q) => (
            <div
              key={q.id}
              className="bg-slate-900 p-6 rounded-2xl border border-slate-800 hover:border-amber-500/40 transition-all space-y-4"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                <div className="md:col-span-8 space-y-1.5">
                  <h3 className="text-lg font-bold text-white">{q.partName}</h3>
                ...
                <div className="md:col-span-4 flex justify-end">
                  <Link
                    href={`/cotacoes/${q.id}`}
                    className="w-full sm:w-auto bg-slate-800 hover:bg-slate-700 text-amber-400 border border-amber-500/30 px-4 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all"
                  >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">Nenhuma cotação encontrada</h3>
